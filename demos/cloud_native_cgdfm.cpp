// cloud_native_cgdfm.cpp: example of BLAS operators using a cloud-native coarse-grain data flow machine
// Created: 2020-03-24
//
...
try {
	...
	openkl::klComputeTargetQuery query = openkl::klComputeTargetQuery{
		openkl::klComputeResource{"",openkl::CN_CGDFM},
	...
	*/
	...
	for (int i = 0; i < targets.size(); ++i) {
		if (targets[i].compute.resourceType == openkl::CN_CGDFM) {
			target = targets[i];
		}
	}

	if (target.compute.resourceType == openkl::COMPUTE_NOP) {
		std::cerr << "Unable to find a Cloud-Native Coarse-grain DFM target: exiting" << std::endl;
		exit(EXIT_FAILURE);
	}

	std::cout << "Successfully selected a CN_CGDFM target\n";
