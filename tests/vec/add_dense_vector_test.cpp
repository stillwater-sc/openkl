          ...
          vtype expected)
{
    ...
    add_dense_vector<posit32>(u_id, v_id, w_id, updater);
    vtype res(3);
    read_dense_vector(u_id, res[0]);
    std::cout << "The result is " << res << '\n';
    check_close(res, expected, "add_dense_vector");
}

int main()
{
    vtype u(3), v{1.1, 2.2, 3.3}, w{4.4, 5.5, 6.6};
    ...
    test(u_id, v_id, w_id, update_store{}, vtype{5.5, 7.7, 9.9});
    test(u_id, v_id, w_id, update_plus{}, vtype{6.5, 8.7, 10.9});
    test(u_id, v_id, w_id, update_minus{}, vtype{-4.5, -6.7, -8.9});
