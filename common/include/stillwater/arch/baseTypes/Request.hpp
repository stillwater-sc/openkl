...
public:
	...
	Payload(uint32_t sizeInBytes, const uint8_t* data = NULL) 
		: m_buffer(new uint8_t[sizeInBytes]), m_sizeInBytes(sizeInBytes), m_bOwnership(true) {
	...
	Payload& operator=(const Payload& rhs) {
		if (m_bOwnership) {
			if (m_sizeInBytes != rhs.m_sizeInBytes) {
				...
				m_buffer		= new uint8_t[m_sizeInBytes];
	 ...
	 */
	void prep(uint32_t sizeInBytes) {
	 ...
	 * of scope.
	 */
	void masquerade(uint32_t sizeInBytes, uint8_t* buffer) {
		if (m_bOwnership && m_buffer != NULL) {
			delete[] m_buffer;
		}
		m_sizeInBytes	= sizeInBytes;
...
private:
	friend std::ostream& operator<<(std::ostream& ostr, const Payload& rhs) {
		...
		if (rhs.m_sizeInBytes > 8) {
			...
			for (uint32_t i = 0; i < rhs.m_sizeInBytes; i++) {
				ostr << hexint(rhs.m_buffer[i]) << ",";
			}
			ostr << "...]";
		}
		else {
			uint32_t last = rhs.m_sizeInBytes-1;
...
public:
	...
	Request(const RequestType type, const RequestTarget target, 
			const Address& address, const uint32_t& sizeInBytes, const uint8_t* data = NULL) 
	...
	uint32_t sizeInBytes() const { return m_payload.sizeInBytes(); }
	const uint8_t* getBuffer() const { return m_payload.getBuffer(); }

	RequestType		m_type;
	RequestTarget	m_target;
	Address			m_address;
	Payload			m_payload;
	uint32_t			m_portId;		// used for debug
private:
...
class Response {
public:
	typedef std::shared_ptr<Response>	SharedPtr;

	Response() : m_status(REQSTATUS_ERROR),m_payload(0) {}
	Response(const RequestStatus& status, uint32_t sizeInBytes, const uint8_t* data = NULL)
	...
	void masquerade(uint32_t sizeInBytes, uint8_t* data) {
